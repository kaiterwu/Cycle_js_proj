...
In cycle, the user is able to click on an element such as the "sunrise/sunset" bar and see a visualization of this data in the form of a circle that will be drawn based on the percentage/value of the statistic being shown. ie, the sunrise/sunset bar will animate based on time of day and will display when sunset is,and will display when sunrise is. Other features include a realtime clock that will animate using setinterval and data retrieval from prevous/future dates. This API allows us to grab data from up to several decades and accurate future forecasts of about 15 days. Users are also able to input their zipcode so to see data for their current location and time. 
